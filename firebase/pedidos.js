...
async function actualizarEstatusPedido(docId, estatus) {
    ...
    await db.collection('negocios').doc(process.env.BUSSINESS_NUMBER).collection('pedidos').doc(docId).update({
        estado: estatus
    }).catch(error => {
...
async function obtenerPedidosCompletados() {
    const snapshot = await db.collection('negocios').doc(process.env.BUSSINESS_NUMBER).collection('pedidos')
        .where('estado', '==', 'pagado')
        .get();
